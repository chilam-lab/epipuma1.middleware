...
* @Last Modified by:   Juan M Barrios
* @Last Modified time: 2018-05-28 23:03:39
*/
...
describe('\n========= Test for getCellScore functionality endpoint =========', function(){

  it('Should return a 200 response - getCellScore', function(done) {
    supertest(server).post('/niche/getCellScore')
      .send({})
      .expect('Content-type',/json/)
      .expect(200, done)
  })

  it('Should respond with a listening message - getCellScore', function(done) {
    supertest(server).post('/niche/getCellScore')
      .send({})
      .expect('Content-type',/json/)
      .expect(200)
      .end(function(err, res) {
        ...
        expect(res.body.message).to.equal('getCellScore endpoint listening, please add the minimum parameters to get a response. See the example parameter')
        done()
      })
  })

  it('Should respond with a example with the minimum parameters required - getCellScore', function(done) {
    supertest(server).post('/niche/getCellScore')
      .send({})
      .expect('Content-type',/json/)
      .expect(200)
