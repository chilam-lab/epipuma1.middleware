...
gridsps as ( 
	...
	from ( 
			-- Verificar si se deben descartas las celdas
			( 
...
apriori as ( 
	select 0 as gridid, 0 as spid, '-'::text as nom_sp, '-'::text as rango, '-'::text as label, ln( ni / ( n - ni::numeric) ) as score 
...
cell_value as ( 
	-- Los conteos peuden ser incorrectos: 
	-- Existen varios casos donde el score es 0 por la diferencia de los conteos con los filtros de fecha
