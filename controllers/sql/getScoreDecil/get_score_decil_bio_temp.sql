with prerawdata as (
	select
		...
		out_score as tscore,
		type_value
	-- from validationprocessbycells($<iterations>, $<spid>, $<alpha>, $<min_occ>, array[$<discardedDeleted:raw>]::int[], '$<res_celda:raw>', '$<where_config:value>', '', 'bio', $<filter_time>, $<caso>, $<lim_inf>, $<lim_sup>, true, '$<fossil:value>', '$<idtabla:raw>')
	from validationprocessbycells(5, 28923, 0.01, 0, array[]::int[], 'gridid_16km', 'where clasevalida = ''Mammalia'' ', '', 'bio', true, 1, 2010, 2020, true, '', 'tabla_temp1')
