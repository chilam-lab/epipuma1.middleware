with rawdata as ( 
	select 
		...
		$<N> as n, -- 6473 as n, 
		round( cast(get_epsilon(cast(cal.nj as integer), cast(sum(cal.Nij) as integer), cast(cel.occ as integer), cast(6473 as integer)) as numeric),2) as epsilon, 
		round( cast(  ln(get_score($<alpha>, cast(cal.nj as integer), cast(sum(cal.Nij) as integer), cast(cel.occ as integer), cast(6473 as integer) ) )as numeric), 2) as score 
	from sp_occ as cel, ( 	
		select 	reinovalido,
				...
				cast('' as text) as rango, (generovalido || ' ' || epitetovalido)  as label, 
