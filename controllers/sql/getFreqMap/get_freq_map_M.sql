with rawdata as ( 
	select 	cal.spid, sum(cal.Nij) as nij,  
			cal.nj,  
			cel.occ as ni,
			$<N> as n,
			-- 6473 as n  , 
...
select 	gsptierra.gridid, 
		case when prgeom.tscore <= -$<maxscore> then 0 when prgeom.tscore >= $<maxscore> then 1 else COALESCE( exp(prgeom.tscore) / (1 + exp(prgeom.tscore)) , exp(val) / (1 + exp(val)) )  end as tscore 
from ( 
	select 	gsp.gridid as gridid,  
			sum(rawdata_scores.score) + ln(rawdata_scores.ni / (rawdata_scores.n - rawdata_scores.ni::numeric)) as tscore 
	from ( 
		select unnest( animalia||plantae||fungi||protoctista||prokaryotae|| animalia_exoticas || plantae_exoticas || fungi_exoticas || protoctista_exoticas || prokaryotae_exoticas || bio01||bio02||bio03||bio04||bio05||bio06||bio07||bio08||bio09||bio10||bio11||bio12||bio13||bio14||bio15||bio16||bio17||bio18||bio19 ||elevacion || pendiente || topidx ) as spid, gridid from gsptierra ) as gsp 
		RIGHT JOIN ( 
			select *  from rawdata 
