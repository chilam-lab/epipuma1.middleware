select 
	cal.spid,cal.reinovalido,cal.phylumdivisionvalido,cal.clasevalida,ordenvalido,cal.familiavalida,cal.generovalido,epitetovalido, 
	cal.label, 
	sum(cal.Nij) as nij,
	cal.nj, 
	cel.occ as Ni, 
	...
	CASE WHEN cal.nj <> 0 
		then 
			round( cast(get_epsilon(cast(cal.nj as integer), cast(sum(cal.Nij) as integer), cast(cel.occ as integer), cast(6473 as integer)) as numeric),2) 
		else 
			0 
	end as epsilon, 
	CASE WHEN cal.nj <> 0 
		then 
			round( cast(  ln(get_score(0.01, cast(cal.nj as integer), cast(sum(cal.Nij) as integer), cast(cel.occ as integer), cast(6473 as integer) ) )as numeric), 2) 
		else 
			0 
	end as score 
from sp_occ as cel, 
( 
	...
	where 	sp_snib.spid <> 49405 
			and clasevalida = 'Mammalia' 
			and epitetovalido <> '' 
	order by spid 
) as cal
where cel.spid =  49405 
group by 	cal.spid, cal.reinovalido, cal.phylumdivisionvalido, cal.clasevalida,ordenvalido, cal.familiavalida, cal.generovalido, epitetovalido,  
			cal.label,  
			cal.nj,  
			cel.occ,  
			n 
